<template>
    <div style="background-color: #cce5ff; padding: 0px 10px;" >
        ...
        <!-- with exactly one real trace loaded there is nothing to choose: the renderer
             auto-generates and pairs the simulated sibling on its own, so the pickers, the
             Add-trace button and the time multiplier would only offer the original-vs-clone
             non-choice. selectDefault (mounted/updated) still runs with this hidden.
             The dropdowns themselves intentionally do NOT set :excludeAutoGenerated — the
             simulated sibling is a meaningful lane choice on this tab only. -->
        <p v-if="realConnectionCount > 1" style="padding-top: 10px;">Select one or more traces via the dropdown(s) below to visualize them in the sequence diagram</p>
        <div v-if="realConnectionCount > 1" class="container-fluid">
                <div class="row">
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        computed: {
            ...
            // are markRaw'd) — the renderer registering its simulated clone never flips this
        ...
        selectDefault(){
            ...
            // never default onto a simulated, autogenerated clone — the renderer pairs those itself
            const defaultConnection = getDefaultRealConnection(this.store.groups);
            if ( defaultConnection !== undefined ){
                this.config.connections.push( SequenceDiagramConfig.createConnectionWithTimeoffset(defaultConnection) );
            }
        },
