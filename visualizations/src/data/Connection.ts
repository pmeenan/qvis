...
export default class QlogConnection {
    ...
    public events:Array<IQlogRawEvent>;

    public lookupTable:Map<string, Map<string, Array<IQlogRawEvent>>>;
    ...
    public eventParser!:IQlogEventParser;

    public constructor(parent:QlogConnectionGroup) {
        markRaw(this);
        this.parent = markRaw(parent);
        this.title = "";
        this.description = "";
        this.events = markRaw(new Array<IQlogRawEvent>());

        this.vantagePoint = markRaw({
        ...
        this.lookupTable = markRaw(new Map<string, Map<string, any>>());
    }
    ...
    public clone():QlogConnection {
        ...
        output.vantagePoint = markRaw(JSON.parse( JSON.stringify(this.vantagePoint || {}) ));
        const events = markRaw(JSON.parse( JSON.stringify(this.events || []) ));
        output.events = events;

        output.eventParser = markRaw(this.eventParser); // TODO: properly clone this one as well! should work for now, since it's supposed to be static
    ...
    public setEventParser( parser:IQlogEventParser ){
        ...
        this.eventParser = markRaw(parser);
    ...
    public lookup(category: qlog.EventCategory | string, eventType: qlog.EventType | string):Array<IQlogRawEvent> {
        // the lookup table is keyed on the normalized category/name the event parsers emit
        // (e.g. draft-era "recovery":"metrics_updated" is stored as "quic":"recovery_metrics_updated")
        // normalize the requested pair through the exact same logic so callers can keep
        // querying with draft-era enum values and still hit spec-final buckets (and vice versa)
        const normalized = normalizeEventPair(category, eventType);

        if ( this.lookupTable.has(normalized.category) && this.lookupTable.get(normalized.category)!.has(normalized.name) ){
            return this.lookupTable.get(normalized.category)!.get(normalized.name)!;
        }
        else {
            return [];
        }
